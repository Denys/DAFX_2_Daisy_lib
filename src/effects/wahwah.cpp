...
float WahWah::Process(const float &in)
{
    ...
    float mod = 0.5f * (1.0f + depth_ * std::sin(2.0f * M_PI * freq_ / sample_rate_));

    // Apply modulated coefficients
    float wc_mod = 2.0f * M_PI * freq_ * mod / sample_rate_;
    float wc_mod_sq = wc_mod * wc_mod;
    ...
    float y0 = b0_mod * in + b1_mod * xnm1_ + b2_mod * xnm2_ -
                a1_mod * ynm1_ - a2_ * ynm2_;
